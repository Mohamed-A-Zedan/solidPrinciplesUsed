            ...
            if (ModelState.IsValid)
            {
                ...
                if (user != null)
                {
                 bool valid=  await userManager.CheckPasswordAsync(user,loginuser.Password);
                    Microsoft.AspNetCore.Identity.SignInResult res =  await signInManager.PasswordSignInAsync(user, loginuser.Password, loginuser.remember_password, false);
                    if (res.Succeeded)
                    {
